		...
		BEGIN
			IF (SALUDOS.existeTipoYNumeroDeDocumento(@username, @documento, @tipo_documento) = 0) --NO EXISTE CLIENTE CON MISMO TIPO Y NRO DE DOCUMENTO
				BEGIN
					...
					INSERT INTO SALUDOS.USUARIOS(USUA_USERNAME, USUA_PASSWORD, USUA_NUEVO, USUA_TIPO)
					VALUES(@username, HASHBYTES('SHA2_256', @password), 1, 'Cliente')
...
BEGIN
	...
	SET @existe = (SELECT COUNT(*)
	FROM SALUDOS.EMPRESAS WHERE EMPR_RAZON_SOCIAL = @razon_social AND EMPR_CUIT = @cuit AND USUA_USERNAME <> @username)
		...
		BEGIN
			IF (SALUDOS.existeRazonSocialYCuit(@username, @razon_social, @cuit) = 0)------No existe empresa con misma razon y cuit
				BEGIN
					...
					SET @fecha_actual = SALUDOS.fechaActual()
					INSERT INTO SALUDOS.USUARIOS(USUA_USERNAME, USUA_PASSWORD, USUA_NUEVO, USUA_TIPO)
					VALUES(@username, HASHBYTES('SHA2_256', @password), 1, 'Empresa')
...
RETURNS TABLE
AS
	RETURN (SELECT USUA_TIPO AS 'Tipo' FROM SALUDOS.USUARIOS WHERE USUA_USERNAME = @username)
GO
...
RETURNS TABLE
AS
	RETURN (SELECT R.ROL_NOMBRE  AS 'Rol' FROM SALUDOS.USUARIOS U, SALUDOS.ROLESXUSUARIO RXU, SALUDOS.ROLES R
			WHERE U.USUA_USERNAME = @username AND U.USUA_USERNAME = RXU.USUA_USERNAME AND R.ROL_COD = RXU.ROL_COD AND RXU_HABILITADO = 1)
GO
...
RETURNS TABLE
AS
	RETURN (SELECT FUNC_NOMBRE AS 'Funcionalidad' FROM SALUDOS.ROLES R, SALUDOS.FUNCIONALIDADESXROL FR, SALUDOS.FUNCIONALIDADES F
...
AS
	RETURN
		(SELECT ROL_COD AS 'Código',ROL_NOMBRE AS 'Nombre',ROL_HABILITADO AS 'Habilitado' FROM SALUDOS.ROLES WHERE	
...
RETURNS TABLE
AS
	RETURN SELECT RUBR_COD AS 'Código' FROM SALUDOS.RUBROS WHERE
...
RETURNS TABLE
AS
	RETURN SELECT EMPR_COD AS 'Código' FROM SALUDOS.EMPRESAS WHERE
...
RETURNS TABLE
AS
	RETURN SELECT CLIE_COD AS 'Código' FROM SALUDOS.CLIENTES WHERE
...
RETURNS TABLE
AS
	RETURN SELECT ITEM_DESCRIPCION AS 'Descripción', ITEM_IMPORTE AS 'Importe', ITEM_CANTIDAD AS 'Cantidad'
...
AS
	RETURN 
		SELECT VISI_COMISION_PUBLICACION AS 'Comisión por Publicación' FROM SALUDOS.VISIBILIDADES
		WHERE VISI_DESCRIPCION = @nombre_visibilidad
GO
...
AS
	RETURN
		SELECT VISI_COMISION_VENTA AS 'Comisión por Venta' FROM SALUDOS.VISIBILIDADES
		WHERE VISI_DESCRIPCION = @nombre_visibilidad
GO
...
AS
	RETURN
		SELECT VISI_COMISION_ENVIO AS 'Comisión por Envío' FROM SALUDOS.VISIBILIDADES
		WHERE VISI_DESCRIPCION = @nombre_visibilidad
GO
